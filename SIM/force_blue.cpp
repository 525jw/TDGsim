#include "force_blue.hpp"

BlueForce::BlueForce(int modelID, Engine* engine, std::string name)
: CoupledModel(modelID, engine)
{
    ...
    logger_system   << "[" << "BlueForce::"
                    << myName
                    << ":Init]"
                    <<" Created, ID : "<<this->GetModelID()
                    <<std::endl;
    
    Company* company = new Company(2, engine, "Company");
    company->SetParentModel(this);

    this->RegisterModelWithID(generator);
    this->RegisterModelWithID(company);

    this->AddInputPort("result");
    this->AddOutputPort("start");

    this->AddCoupling(generator,"start",this,"start",EOC);
    this->AddCoupling(this,"result",company,"result",EIC);
    this->AddCoupling(company,"restart",generator,"restart",IC);
...
void BlueForce::ReceiveScheduleTime(const TIME_T currentTime) {
    logger_system   << "[" << "BlueForce::"
                    << myName
                    << "::ReceiveScheduleTime]"
...
void BlueForce::ReceiveEvent(Event& event, TIME_T currentTime) {
    logger_system   << "[" << "BlueForce::"
                    << myName
                    << "::ReceiveEvent]"
...
const TIME_T BlueForce::QueryNextTime() const {
    logger_system   << "[" << "BlueForce::"
                    << myName
                    << "::QueryNextTime]"
                    <<" query min TA"
