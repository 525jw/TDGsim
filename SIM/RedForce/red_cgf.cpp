...
void RedCgf::RebuildEnemyPosList() {
    enemyIds.clear();
...
bool RedCgf::ExtTransFn(const std::string& inPort, const std::any& anyMessage) {
    if (inPort == "Start") {
        Start message;
...
bool RedCgf::OutputFn(){
    if (this->GetCurState() == "FIRE"){
        ...
        if (roll<=this->accuracy){
            ...
            logger_world << "["<<this->info.name<<"] "<< "shoots " <<env->QueryEntityById(targetId)->name<<" when Time : "<<this->engine->GetCurrentTime()<<std::endl;
